...
const SearchSkills = () => {
  ...
  const handleSearch = async (e) => {
    ...
    try {
      const response = await fetch('http://localhost:3000/api/search-skills', {
        method: 'POST',
        headers: {
      ...
      if (!response.ok) {
        throw new Error('Skill not found. Please try a different term.');
      }
    ...
    <div className=" min-h-screen bg-gray-100 dark:bg-gray-900 text-gray-900 dark:text-gray-100 p-8 flex flex-col items-center">
      ...
      <div className="w-full max-w-4xl bg-white dark:bg-gray-800 shadow-xl rounded-2xl p-8 mb-8">
        <h1 className="text-4xl font-extrabold text-center mb-6 text-gray-900 dark:text-gray-100 ">
          Skill Occupations Finder
        </h1>
        <p className="text-center text-gray-600 dark:text-gray-400 mb-8">
          Enter a skill to find which occupations require it.
        </p>
        <form onSubmit={handleSearch} className="flex flex-col md:flex-row gap-4">
          ...
          <button
            type="submit"
            className="p-4 rounded-full bg-green-800 text-white font-bold hover:bg-blue-700 transition-colors shadow-lg"
            disabled={loading}
          >
        ...
        <div className="w-full max-w-4xl grid gap-8 md:grid-cols-2 mt-8">
          ...
          <div className="bg-white dark:bg-gray-800 p-6 rounded-2xl shadow-lg md:col-span-2">
            <h2 className="text-2xl font-bold mb-4 border-b-2 pb-2">
              <span className="text-blue-600">Skill:</span> {results.skill.PREFERREDLABEL}
            </h2>
            <p className="text-gray-600 dark:text-gray-400">
              {results.skill.DESCRIPTION || 'No description available.'}
            </p>
          </div>
          ...
          <div className="bg-white dark:bg-gray-800 p-6 rounded-2xl shadow-lg">
            <h3 className="text-xl font-bold mb-4">
              <span className="text-green-600">Occupations Requiring this Skill</span>
            </h3>
            <ul className="list-disc pl-5 space-y-2">
              {results.occupationsRequiringSkill.length > 0 ? (
                results.occupationsRequiringSkill.map(occ => (
                  <li key={occ.ID} className="text-gray-700 dark:text-gray-300">
                    <span className="font-semibold">{occ.PREFERREDLABEL}</span>
                  </li>
                ))
              ) : (
